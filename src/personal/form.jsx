        ...
        <div className="auth-card">
          {!isVerifying ? (
            ...
            <>
              ...
              <input type="text" className="auth-email-display" value={email} readOnly />
              {error && <div className="auth-error" style={{ marginBottom: '14px' }}>{error}</div>}
                ...
                <div className="auth-otp-boxes">
                  {digits.map((d, i) => (
                    <input key={i} ref={el => inputRefs.current[i] = el}
                      ...
                      className="auth-otp-box" value={d}
                      onChange={e => handleDigitChange(i, e.target.value)}
                      onKeyDown={e => handleDigitKeyDown(i, e)} />
                  ))}
                </div>
                <p className="auth-expire-text">Code expires in <span>15min</span></p>
                <button type="submit" className="auth-submit" disabled={loading}>
                  {loading ? "Verifying..." : "Verify Email"}
                </button>
              </form>
              <p className="auth-resend">
                Didn't receive it? <button type="button" onClick={async () => {
                  try { await API.post('/auth/register', { name, email, password }); } catch {}
                }}>Resend Code</button>
              </p>
              <button type="button" className="auth-back-link" onClick={() => setIsVerifying(false)}>
